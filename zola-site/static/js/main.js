...
// Stream sources — user can pick, preference saved in cookie
const STREAMS = [
  { id: 'mega-hifi', label: 'Mega Relay', detail: 'AAC 320k', url: 'https://zamrock-relay.mega-utilities.com/hifi.aac' },
  { id: 'original', label: 'Original Stream', detail: 'MP3 192k', url: 'https://divine-paper-3624.deathsmack-a51.workers.dev/' },
]
const DEFAULT_STREAM = 'mega-hifi'
const FALLBACK_STREAM = 'original'
...
function getSavedStream () {
  try {
    const m = document.cookie.match(/(?:^|;\s*)zamrock-stream=([^;]*)/)
    return m ? m[1] : null
  } catch { return null }
}

function saveStream (id) {
  try { document.cookie = `zamrock-stream=${id}; path=/; max-age=31536000; SameSite=Lax` } catch {}
}

function getStream (id) { return STREAMS.find(s => s.id === id) || STREAMS[0] }

function getActiveStream () { return getStream(usingFallback ? FALLBACK_STREAM : currentStreamId) }

function applyStream (audio, streamId) {
  currentStreamId = streamId
  usingFallback = false
  audio.src = getStream(streamId).url
  audio.load()
  saveStream(streamId)
}
...
function getPlayLabel () {
  ...
  return usingFallback ? `${s.label} (fallback)` : `${s.label} (${s.detail})`
}
...
function setPlayerStatus (status, button) {
  ...
  switch (status) {
    case 'idle':
      button.textContent = playerState.hasPlayedBefore ? 'Play' : getPlayLabel()
...
function reconnectStream (audio, button) {
  if (playerState.reconnectAttempts >= playerState.maxReconnectAttempts) {
    if (!usingFallback && currentStreamId !== FALLBACK_STREAM) {
      console.warn('Max reconnection attempts on primary, switching to fallback')
      usingFallback = true
      audio.src = getStream(FALLBACK_STREAM).url
      playerState.reconnectAttempts = 0
      audio.load()
      audio.play().then(() => {
        playerState.reconnectAttempts = 0
    ...
    console.warn('Max reconnection attempts on fallback too, giving up')
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (audio && playButton && volumeSlider) {
    // Restore saved stream preference or use default
    const saved = getSavedStream()
    applyStream(audio, saved && STREAMS.find(s => s.id === saved) ? saved : DEFAULT_STREAM)
    playerState.initialText = getPlayLabel()
    ...
    if (streamSelect) {
      ...
      streamSelect.addEventListener('change', () => {
        const wasPlaying = playerState.status === 'playing' || playerState.wasPlaying
        applyStream(audio, streamSelect.value)
        playerState.initialText = getPlayLabel()
        playerState.reconnectAttempts = 0
        if (playerState.reconnectTimer) {
          clearTimeout(playerState.reconnectTimer)
        ...
        if (wasPlaying) {
          ...
          setPlayerStatus('idle', playButton)
        }
      })
    }
